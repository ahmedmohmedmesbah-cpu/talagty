(() => {
    ...
    async function bootstrap() {
        if (previewMode) { Object.assign(state, structuredClone(demo)); showApp(); renderAll(); return; }
    ...
    async function saveProduct(event) { event.preventDefault(); const id = $('product-id').value, existing = state.products.find(item => String(item.id) === id), body = { sku: $('product-sku').value.trim(), name_ar: $('product-name').value.trim(), description_ar: $('product-description').value.trim(), category_id: $('product-category').value ? Number($('product-category').value) : null, unit_price: Number($('product-price').value), image_url: normalizeDriveUrl($('product-image').value.trim()), sale_type: existing?.sale_type || 'none', sale_value: Number(existing?.sale_value || 0), sale_start: existing?.sale_start || null, sale_end: existing?.sale_end || null, stock_quantity: Number($('product-stock').value || 0), low_stock_threshold: Number($('product-low-stock').value || 0), is_active: $('product-active').checked }; try { if (previewMode) { if (existing) Object.assign(existing, body, { category_name: state.categories.find(category => Number(category.id) === Number(body.category_id))?.name_ar || 'بدون فئة' }); else state.products.push({ ...body, id: Date.now(), category_name: state.categories.find(category => Number(category.id) === Number(body.category_id))?.name_ar, effective_price: body.unit_price }); } else await api(id ? `/api/admin/products/${id}` : '/api/admin/products', { method: id ? 'PATCH' : 'POST', body }); closeModals(); await refreshView(); toast(id ? 'تم تعديل المنتج' : 'تمت إضافة المنتج'); } catch (error) { formMessage(event.currentTarget, error.message, true); } }
    ...
    async function saveSupplier(event) { event.preventDefault(); const body = { full_name: $('supplier-name').value.trim(), phone: $('supplier-phone').value.trim(), email: $('supplier-email').value.trim() || null, password: $('supplier-password').value, national_id: $('supplier-national-id').value.trim(), vehicle_details: $('supplier-vehicle').value.trim(), business_name: $('supplier-name').value.trim() }; try { let result; if (previewMode) { result = { activation_code: '482915' }; state.suppliers.push({ supplier_id: Date.now(), ...body, is_available: true, is_activated: false }); } else result = await api('/api/admin/suppliers', { method: 'POST', body }); formMessage(event.currentTarget, `تم إنشاء الحساب. رمز التفعيل لمرة واحدة: ${result.activation_code}`, false); renderSuppliers(); toast('تم إنشاء حساب المورد'); } catch (error) { formMessage(event.currentTarget, error.message, true); } }
