(() => {
    ...
    const demo = {
        ...
        orders: [{ order_id: 'demo-1', order_number: 'TLG-20260827-A12B3C4D', customer_name: 'محمود السيد', customer_phone: '01002500273', customer_address_text: 'مدينة نصر، شارع الطيران، عمارة ١٢', admin_note: 'الاتصال بعد الساعة الخامسة', status: 'pending_assignment', customer_device_bound: false, total: 116, created_at: new Date().toISOString(), items: [{ item_id: 1, product_id: 'mc1', name: 'حليب كامل الدسم', quantity: 2, unit_price: 18, line_total: 36 }, { item_id: 2, product_id: 'ln1', name: 'لانشون بيتزا', quantity: 2, unit_price: 40, line_total: 80 }] }, { order_id: 'demo-2', order_number: 'TLG-20260827-E56F7G8H', customer_name: 'سارة محمد', customer_phone: '01111111111', customer_address_text: 'التجمع الخامس، الحي الأول', status: 'preparing', customer_device_bound: false, assigned_supplier_name: 'أحمد محمود', total: 180, created_at: new Date(Date.now() - 3600000).toISOString(), items: [{ item_id: 3, product_id: 'ch1', name: 'جبن شيدر', quantity: 4, unit_price: 40, line_total: 160 }] }, { order_id: 'demo-3', order_number: 'TLG-20260826-K91L2M3N', customer_name: 'عمر حسن', customer_phone: '01222222222', customer_address_text: 'المعادي، شارع ٩', status: 'completed', customer_device_bound: true, assigned_supplier_name: 'محمد علي', total: 250, created_at: new Date(Date.now() - 86400000).toISOString(), items: [{ item_id: 4, product_id: 'ln1', name: 'لانشون بيتزا', quantity: 5, unit_price: 50, line_total: 250 }] }]
    };
    ...
    async function sendCustomerActivation(orderId, button) {
        const order = state.orders.find(candidate => String(candidate.order_id) === String(orderId)); if (!order) return;
        let whatsappWindow = null;
        if (!previewMode) whatsappWindow = window.open('about:blank', '_blank');
        button.disabled = true; const originalLabel = button.textContent; button.textContent = 'جاري تجهيز الرسالة…';
        try {
            const result = previewMode ? { activation_code: '482915', expires_at: new Date(Date.now() + 600000).toISOString(), masked_phone: '0100••••273', whatsapp_url: '#' } : await api(`/api/admin/orders/${encodeURIComponent(orderId)}/customer-activation`, { method: 'POST', body: {} });
            const resultBox = $('customer-activation-result');
            resultBox.innerHTML = `<strong>تم إنشاء كود التفعيل</strong><span>العميل: ${escapeHtml(result.masked_phone)}</span><b>${escapeHtml(result.activation_code)}</b><small>صالح حتى ${formatDate(result.expires_at)}. افتح واتساب ثم اضغط إرسال.</small>${previewMode ? '<span>هذه معاينة فقط، ولن يتم فتح واتساب.</span>' : `<a class="btn btn-whatsapp" href="${escapeHtml(result.whatsapp_url)}" target="_blank" rel="noopener">فتح واتساب مرة أخرى</a>`}`;
            resultBox.hidden = false;
            button.hidden = true;
            if (whatsappWindow) whatsappWindow.location.href = result.whatsapp_url;
            toast('تم تجهيز رسالة واتساب. اضغط إرسال داخل واتساب.');
        } catch (error) {
            if (whatsappWindow) whatsappWindow.close();
            button.disabled = false; button.textContent = originalLabel; toast(error.message, true);
        }
    }
    ...
    async function removeSupplier(button) {
        const supplierId = Number(button.dataset.removeSupplier);
        const supplier = state.suppliers.find(item => item.supplier_id === supplierId);
        if (!supplier || button.disabled) return;
        if (!confirm(`هل تريد إزالة المندوب «${supplier.full_name}»؟\nسيُمنع من الدخول وتُلغى جلساته، مع الاحتفاظ بطلباته وفواتيره السابقة. لا يمكن إزالته إذا كانت لديه طلبات نشطة.`)) return;
        button.disabled = true;
        try {
            if (previewMode) {
                const active = state.orders.some(order => !['completed','cancelled'].includes(order.status) &&
                    (order.assignments?.some(assignment => Number(assignment.supplier_id) === supplierId) || order.assigned_supplier_name === supplier.full_name));
                if (active) throw new Error('لا يمكن إزالة المندوب لأن لديه طلبات نشطة. أكمل معالجة طلباته أولاً');
            } else await api(`/api/admin/suppliers/${supplierId}`, { method: 'DELETE' });
            state.suppliers = state.suppliers.filter(item => item.supplier_id !== supplierId);
            renderSuppliers(); fillSelects(); closeModals();
            toast('تمت إزالة حساب المندوب وإيقاف دخوله مع حفظ السجل السابق');
    ...
    document.addEventListener('click', event => {
        ...
        const removeEmployee = event.target.closest('[data-remove-supplier]'); if (removeEmployee) removeSupplier(removeEmployee);
    });
